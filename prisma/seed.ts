...
async function main() {
  ...
  const events = await Promise.all(
    Array.from({ length: 40 }).map(async () => {
      const theme =
        politicalThemes[Math.floor(Math.random() * politicalThemes.length)];
      ...
      const endAt = new Date(
        startAt.getTime() + (Math.random() * 3 + 1) * 60 * 60 * 1000
      );
      ...
      return prisma.event.create({
        data: {
          ...
          picture: `/images/political-event-${
          ...
          endAt,
          status:
            Math.random() > 0.2 ? EventStatus.PUBLISHED : EventStatus.DRAFT,
        ...
        Array.from({ length: photoCount }).map(async () => {
          ...
          return prisma.photo.create({
            data: {
              ...
              url: `/images/event-photos/event-photo-${
